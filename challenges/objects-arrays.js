...
console.log('The Tyrannosaurus weighed ' +trex.weight);

// What was the diet of a velociraptor?
console.log('The Velociraptors diet was '
  +raptor.diet);

// How long was a stegosaurus?
console.log('The Stegosaurus was '
   +steg.length +' long');

// What time period did tyrannosaurus live in?
console.log('The Tyrannosaurus lived in the '
  +trex.period +' period');
...
graduates.forEach((first) =>{
  universities.push(first.university)
});
...
graduates.forEach((first) => {
  contactInfo.push(`${first.first_name} ${first.email}`)
});
...
for (index = 0; index < graduates.length; ++index) {
  value = graduates[prop];
  if (value.substring(0, 2) == "A") {
    result = value;
    break;
  }
}
...
let hasUni = graduates.filter(function(graduates){
    return graduates.university === 'University';
  });

unisWithUni.push(hasUni);
...
let caps = zooAnimals.map(letsCap);

function letsCap(lower) {

  return lower.animal_name.charAt(0).toLowerCase() + lower.animal_name.slice(1);

}

lowCaseAnimalNames.push(caps);
...
animalPopulationTotal = populationTotal.reduce(calc);
